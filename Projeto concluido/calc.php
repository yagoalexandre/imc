<!-- Pagina que retorna o resultado de seu imc e seu grau de obesidade. -->
...
<?php /* Código do calculo do IMC */
    $altura = $_GET['altura'];
    $peso = $_GET['peso'];
    $calc = $peso/pow($altura,2) ; // faz o calculo do imc
    $imc = (number_format($calc,1)); // limita e arredonda as casas decimais para o IMC
    echo '<h1>Seu imc é de ' .$imc. '</h1>';  // mostra o calculo do imc
?>
...
<body>
    ...
    <header>
        <?php  /* Funções especificar para cada grau de obesidade. */
            if ($imc < 18.5) {
            ...
            elseif ($imc > 34.9 and $imc < 39.8) {
                echo"<h1>Você está em um grau servero de obesidade, talvez deva procurar ajudar médica e comece um tratamento.</h1>";
            }
            elseif ($imc > 40.0) {
                echo"<h1>Você está com sérios riscos a sua saúde, procure por ajuda médica urgentemente.</h1>";
            }
